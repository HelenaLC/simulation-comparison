# wcs <- list(stat1d = "ks")
# args <- list(
#     fun = "code/utils-plotting.R",
#     res = "outs/obj-stat_1d.rds",
#     rds = sprintf("plts/stat_1d_by_stat1d-correlations_by_method,%s.rds", wcs$stat1d),
#     pdf = sprintf("plts/stat_1d_by_stat1d-correlations_by_method,%s.pdf", wcs$stat1d))

source(args$fun)
res <- readRDS(args$res)

mat <- res %>% 
    # keep data of interest
    filter(stat1d == wcs$stat1d) %>% 
    .filter_res() %>% 
    # for each method, method & summary, average across groups
    group_by(refset, method, metric, group) %>% .avg(n = 1) %>% 
    # for each method, correlate b/w summaries & across refsets
    split(.$method) %>% 
    lapply(\(.)
        pivot_wider(.,
            names_from = metric, 
            values_from = stat) %>% 
        select(any_of(.metrics_lab)) %>% 
        cor(method = "spearman", 
            use = "pairwise.complete.obs"))

df <- lapply(mat, \(u) u %>% 
    data.frame(from = rownames(.)) %>% 
    pivot_longer(-from, names_to = "to")) %>% 
    bind_rows(.id = "method") %>% 
    complete(from, to, method, fill = list(value = NA))
    
foo <- df %>% 
    group_by(from, to) %>% 
    summarise(value = mean(value), .groups = "drop") %>% 
    pivot_wider(names_from = to, values_from = value) %>% 
    select(-from)
foo[is.na(foo)] <- 1
o <- colnames(foo)[hclust(dist(as.matrix(foo)))$order]

plt <- ggplot(df, aes(from, to, fill = value)) +
    facet_wrap(~ method, nrow = 4) +
    scale_fill_distiller(
        bquote("r("*.(.stats1d_lab[wcs$stat1d])*")"),
        palette = "RdYlBu", 
        na.value = "grey",
        limits = c(-1, 1), 
        n.breaks = 3) +
    scale_x_discrete(limits = rev(o), labels = .metrics_lab) +
    scale_y_discrete(limits = o, labels = .metrics_lab) +
    coord_equal(expand = FALSE) +
    geom_tile()

thm <- theme(
    axis.title = element_blank(),
    axis.text.x = element_text(angle = 45, hjust = 1))

fig <- .prettify(plt, thm)

saveRDS(fig, args$rds)
ggsave(args$pdf, fig, width = 16, height = 16, units = "cm")
