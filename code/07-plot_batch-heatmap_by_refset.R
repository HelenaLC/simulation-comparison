...
#     rds = paste0("plts/batch-heatmap_by_refset_", wcs$val, ".rds"),
#     pdf = paste0("plts/batch-heatmap_by_refset_", wcs$val, ".pdf"),
...
plt <- ggplot(df, aes(
    reorder_within(batch_method, .data[[wcs$val]], refset, median), 
    sim_method, fill = .data[[wcs$val]])) +
    facet_grid(~ refset, scales = "free_x") +
    geom_tile(col = "white") +
