...
#     rds = sprintf("plts/stat_1d_by_reftyp-pca,%s,%s.rds", wcs$reftyp, wcs$stat1d),
#     pdf = sprintf("plts/stat_1d_by_reftyp-pca,%s,%s.pdf", wcs$reftyp, wcs$stat1d))
...
xy <- pc$x %>% 
    ...
    mutate(
        ...
        reftyp = res$reftyp[match(refset, res$refset)]) %>% 
    mutate_at("reftyp", factor, c("n", "b", "k"))
